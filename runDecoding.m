...
allModels = cell(length(parts), nFolds);
...
for p = 1:length(parts)
    ...
    for f = 1:nFolds
        ...
        % Train model
        allModels{p, f} = trainModel(trainFeatures, trainLabels, classifier, classifierParam);

        % Evaluate model
        predLbl(fold == f & labels > 0) = generatePredictions(testFeatures, allModels{p,f}, classifier);
